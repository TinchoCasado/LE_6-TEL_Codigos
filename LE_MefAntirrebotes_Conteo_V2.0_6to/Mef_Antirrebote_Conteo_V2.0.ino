...
int print(int, char);          //se ejecuta al presionar el botón para imprimir en el display
...
void setup ()
{
  ...
  t_now = millis();
  print(0); //El display cuenta cada vez que el botón es presionado
...
void loop()
{
  ...
  if (botonPres)
  {
    cuenta=print(cuenta); //El display cuenta cada vez que el botón es presionado
    botonPres = false;
  }
  //PORTD = 0b00110011;
}
...
int print(int valor)    //Función para imprimir los dígitos
{
  PORTD = pepito[valor++];
  if (valor>VAL_MAX)valor=0;
  return valor;
}
